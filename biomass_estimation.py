...
print(f"5-Fold CV R² scores: {cv_r2}")
...
print(f"OOB R² (internal validation): {rf.oob_score_:.4f}")
...
print(f"Bootstrap 95% CI R²: {ci_r2}")
...
print(f"Bootstrap RMSE 95% CI: {rmse_ci[0]:.2f} – {rmse_ci[1]:.2f} → {rmse_mean:.2f} ± {rmse_range:.2f}")
print(f"Bootstrap R² 95% CI: {r2_ci[0]:.3f} – {r2_ci[1]:.3f} → {r2_mean:.3f} ± {r2_range:.3f}")
...
model = rf
print(f"✅ gpr model loaded: {type(model)}")
...
with rasterio.open(raster_path) as src:
    ...
    for row_start in range(0, rows, tile_size):
        for col_start in range(0, cols, tile_size):
            ...
            print(f"✅ Processed rows {row_start}:{row_end}, cols {col_start}:{col_end}")
...
print(f"🎯 Saved regression raster to: {out_classified_tif}")
...
# 1️⃣ Compute mean, std, and count per plot
df_grouped = (
...
# 2️⃣ Compute 95% Confidence Intervals
...
# 3️⃣ Plot side-by-side bars with error bars (95% CI)
...
# 1️⃣ Compute group-wise mean, std, and count
df_grouped = (
    merged_df.groupby('Plot_ID_x', as_index=False)
    .agg({
        'pixel_biomass_Mgha': ['mean', 'std', 'count'],
...
# 3️⃣ Compute performance metrics (using means)
...
print("📊 Model Performance by Plot (using plot means)")
print(f"RMSE: {rmse:.2f}")
print(f"MAE:  {mae:.2f}")
print(f"Bias: {bias:.2f}")
print(f"R²:   {r2:.3f}")

# 4️⃣ Side-by-side bar plot with 95% CI
...
# 6️⃣ Residuals per plot (for diagnostic)
...
# 7️⃣ Compute percent error per plot
df_grouped['Percent_Error'] = (
    (df_grouped['Predicted_mean'] - df_grouped['Observed_mean'])
    / df_grouped['Observed_mean']
) * 100

# 8️⃣ Determine whether the model overestimated or underestimated per plot
...
# 9️⃣ Create clean text report
print("\n📄 Biomass Estimation Summary by Plot:\n")
